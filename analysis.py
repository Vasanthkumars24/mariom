...
def _():
    ...
    return functools, mo, np, plt


@app.cell
def _(functools, np, plt):
    @functools.cache
...
@app.cell
def _(amplitude, mo, np, period):
    mo.md(
        f"""
        Lets's plot $f(x) = {amplitude.value} \cdot \sin ({2 * np.pi / period:.02f}x)$
        """
    )
    return
...
@app.cell
def _(amplitude, period, plotsin):
